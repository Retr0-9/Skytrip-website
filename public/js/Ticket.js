...
const API = {
  ...
  endpoints: {
    myPaidTickets: "/api/tickets/my",
    myTicketDetails: (ticketId) =>
      `/api/tickets/my/${encodeURIComponent(ticketId)}`,

    cancelTicket: (ticketId) =>
      `/api/tickets/my/${encodeURIComponent(ticketId)}/cancel`,
...
function normPaidRow(x) {
  const bookingStatus = (x.bookingStatus ?? x.BookingStatus ?? "").toString();

  const paymentStatus = (x.paymentStatus ?? x.PaymentStatus ?? "").toString();
  ...
  return {
    ...
    passengersCount: x.passengersCount ?? x.PassengersCount ?? x.count ?? 0,
...
function cardHTML(r) {
  ...
  if (!r.isPaid) {
    statusBadge = '<span class="badge danger">Not Paid</span>';
  } else if (r.isConfirmed) {
    statusBadge = '<span class="badge success">Confirmed</span>';
  } else {
  ...
  <button class="btn primary" data-act="details">
    📄 View Details
  </button>
  ...
  for (const r of list) {
    ...
    if (cancelBtn) {
      cancelBtn.addEventListener("click", async () => {
        const ok = confirm(`Cancel ticket #${r.ticketId}?`);

        if (!ok) return;

        try {
          await apiFetch(API.endpoints.cancelTicket(r.ticketId), {
            method: "POST",
          });
...
function applyFilters() {
  ...
  if (STATUS_FILTER === "confirmed") {
    list = list.filter((x) => x.isConfirmed);
  }
  ...
  if (STATUS_FILTER === "pending") {
    list = list.filter(
      (x) =>
        x.isPaid &&
        !x.isConfirmed &&
        x.bookingStatus.trim().toLowerCase() !== "cancelled",
    );
  }

  if (STATUS_FILTER === "cancelled") {
    list = list.filter(
      (x) => x.bookingStatus.trim().toLowerCase() === "cancelled",
    );
  }
...
async function openDetails(ticketId) {
  try {
    ...
    $("mSub").textContent = `${safe(summary.bookingStatus)} • ${fmtDateTime(
      summary.bookingDate,
    )}`;
...
$("statusTabs").addEventListener("click", (e) => {
  ...
  STATUS_FILTER = btn.dataset.statusFilter;
