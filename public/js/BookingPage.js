      ...
      document.getElementById("pclassPath").textContent =
        ENDPOINTS.passengerClassAll;
      ...
      function dec(type) {
        ...
        pax[type] -= 1;
        if (type === "adults" && pax.infants > pax.adults)
      ...
      function ensureMinLegs() {
        if (legs.length < 2)
          while (legs.length < 2) legs.push(createLegModel());
      ...
      async function initLookups() {
        ...
        if (pclassRes.status === "fulfilled") {
          ...
          plist.forEach((pc) => {
            ...
            const name =
              pc.nameClass ?? pc.NameClass ?? pc.name ?? pc.title ?? "";
