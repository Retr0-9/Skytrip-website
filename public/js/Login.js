...
form.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    // =========================
    let userId =
      data?.userId ?? data?.userID ?? data?.UserId ?? data?.UserID ?? null;
    let clientId =
      data?.clientId ??
      data?.clientID ??
      data?.ClientId ??
    ...
    if (jwt) {
      ...
      personId = personId ?? pick(jwt, "personId", "personID", "PersonId");
      clientId = clientId ?? pick(jwt, "clientId", "clientID", "ClientId");
    }
    ...
    if (returnedEmail) localStorage.setItem("email", String(returnedEmail));
