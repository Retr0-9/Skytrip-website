...
const paxCount = Number(sessionStorage.getItem("totalPassengers") || "1") || 1;
const classFee = Number(sessionStorage.getItem("selectedClassFee") || "0") || 0;
...
function normalizeResults(r) {
  ...
  if (
    Array.isArray(base) &&
    (base.length === 0 || (base[0] && (base[0].segments || base[0].Segments)))
  ) {
    return {
...
function computeTotal(it) {
  ...
  const baseSum = segs.reduce((sum, s) => sum + (Number(s.basePrice) || 0), 0);
...
function onSelectItinerary(it) {
  ...
  sessionStorage.setItem("selectedFlightScheduleIds", JSON.stringify(ids));
