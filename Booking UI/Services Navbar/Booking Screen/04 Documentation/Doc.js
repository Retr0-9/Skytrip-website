...
// ========= CONFIG =========
const BASE_URL =
  sessionStorage.getItem("API_BASE_URL") || "https://localhost:7216";

// جرّب أكثر من Prefix حسب مشروعك
...
function showToast(msg, ok = true) {
  const t = $("toast");
  t.innerHTML = ok ? `✅ <b>Success:</b> ${msg}` : `⚠️ <b>Error:</b> ${msg}`;
...
function isoFromDateOnly(dateStr) {
  if (!dateStr) return "";
  // بدون Z عشان ما تعمل مشاكل timezone
...
function validateFormHard() {
  ...
  if (!file) return { ok: false, msg: "Please select a document (PDF/Photo)." };
...
async function getBookingPassengersSmart(bookId) {
  ...
  for (const makePath of PASSENGERS_ENDPOINT_CANDIDATES) {
    ...
    try {
      ...
      // خزّن المسار اللي نجح عشان نستخدمه POST كمان
...
function renderCountries() {
  ...
  for (const c of countries) {
    ...
    opt.textContent = c.countryName ?? c.CountryName ?? c.name ?? "Country";
...
function normalizeBookingCounters(data) {
  ...
  const created = expected > 0 ? Math.min(createdRaw, expected) : createdRaw;
...
function renderBooking() {
  ...
  const bookVal = bookingData.bookId ?? bookingData.BookId ?? bookId ?? "—";
  const ticketVal =
    bookingData.ticketId ?? bookingData.TicketId ?? ticketId ?? "—";

  $("kvBook").textContent = bookVal;
  $("kvTicket").textContent = `Ticket #${ticketVal}`;
  ...
  $("kvExpected").textContent = expected || "—";
  $("kvCreated").textContent = createdRaw;

  const currentIndex = done ? expected : Math.min(created + 1, expected || 1);

  $("progressText").textContent = expected
    ? `${currentIndex} of ${expected} passengers`
    : "Passengers";

  const percent = expected > 0 ? Math.min(100, (created / expected) * 100) : 0;
  ...
  if (pax.length === 0) {
    list.innerHTML = `
        <div class="pax">
          <div>
            <div class="name">No passengers yet</div>
            <div class="meta">Start filling the form to add passengers.</div>
          </div>
          <div class="tag bad">Empty</div>
        </div>`;
  } else {
    for (const p of pax) {
      const full = [p.firstName, p.secondName, p.thirdName, p.lastName]
        .filter((x) => x && String(x).trim().length)
        .join(" ");

      const issueId = Number(p.issueCountryId ?? p.IssueCountryId ?? 0);
      const countryName =
        countries.find((x) => Number(x.countryID ?? x.CountryID) === issueId)
          ?.countryName || (issueId ? `Country #${issueId}` : "—");

      const docOk = !!(p.hasDocument ?? p.HasDocument);

      const metaLines = [
        `Gender: ${p.gender || p.Gender || "—"}`,
        `DOB: ${formatDateOnly(p.birthDate ?? p.BirthDate)}`,
        `Issue Country: ${countryName}`,
        `Doc: ${p.documentationType ?? p.DocumentationType ?? "—"}`,
      ].join(" • ");
      ...
      el.innerHTML = `
          <div>
            <div class="name">#${p.passengerId ?? p.PassengerId ?? "—"} — ${escapeHtml(full || "Passenger")}</div>
            <div class="meta">${escapeHtml(metaLines)}</div>
          </div>
          <div class="tag ${docOk ? "ok" : "bad"}">${docOk ? "Has Document" : "No Document"}</div>
        `;
      list.appendChild(el);
    }
  }
...
async function refreshAll() {
  ...
  try {
    ...
    if (!$("issueCountryId").value && countries.length) {
      const firstId = countries[0].countryID ?? countries[0].CountryID ?? "";
      if (firstId) $("issueCountryId").value = String(firstId);
    }

    // حاول تلتقط ticketId لو API رجّعه
    const tFromApi =
...
async function submitPassenger(e) {
  ...
  if (!bookingData) return showToast("Booking data not loaded yet.", false);
  ...
  try {
    ...
    // ✅ names match typical Swagger DTO (case-insensitive غالباً)
    ...
    fd.append("ExpirationDate", isoFromDateOnly($("expirationDate").value));
    ...
    const { expected: e2, created: c2 } = normalizeBookingCounters(bookingData);
    if (e2 > 0 && c2 >= e2) {
