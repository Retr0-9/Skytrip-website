...
const ENDPOINTS = {
  ...
  getTicketByBooking: (bookId) => `/api/InfoTickets/ByBooking/${bookId}`,
...
// (اختياري) mapping لأسماء المدن/الدول — خليها كما هي أو عدّلها لاحقاً
const CITY_NAME_MAP = {
  AMM: "AMM",
  UK: "UK",
  KSA: "KSA",
...
function readSelectedItinerary() {
  const it = safeJsonParse(sessionStorage.getItem("selectedItinerary"), null);
  if (it && Array.isArray(it.segments)) return it;

  const f = safeJsonParse(sessionStorage.getItem("selectedFlight"), null);
  if (f) {
    return {
      totalPrice: null,
      segments: [
        {
          ...
          from: pick(f, "from", "From", "departureCity", "DepartureCity"),
  ...
  if (id) return { totalPrice: null, segments: [{ flightScheduleId: id }] };
...
async function loadSchedulesForSegments(segments) {
  ...
  for (const seg of segments) {
    ...
    const depCity = pick(seg, "departureCity", "DepartureCity", "from", "From");
    ...
    loaded.push({
      ...
      arrivalCity: cityLabel(pick(fs, "arrivalCity", "ArrivalCity") ?? "—"),
...
function renderHeader(tripType, schedules) {
  ...
  const route =
    n > 0 ? `${first.departureCity ?? "—"} → ${last.arrivalCity ?? "—"}` : "—";
    ...
    el.innerHTML = `
        <div class="seg-top">
          ...
          <div class="seg-actions">
            <button class="chip" data-act="seat" data-idx="${idx}">Seat Map</button>
  ...
  host.onclick = (e) => {
    ...
    if (act === "seat") {
      window.location.href =
        "SeatMap.html?id=" + encodeURIComponent(String(s.flightScheduleId));
    }
    ...
    if (act === "focus") {
      ...
      $("duration").textContent = calcDuration(s.departureTime, s.arrivalTime);
...
async function getPassengerClassFee(passengerClassID) {
  ...
  const pc = await apiGet(ENDPOINTS.passengerClassById(passengerClassID));
  return (
    Number(
      pc?.feesClass ??
        pc?.FeesClass ??
...
async function renderPriceForItinerary(itinerary, schedules) {
  const pax = Number(sessionStorage.getItem("totalPassengers") || "1") || 1;
  ...
  // ✅ total محسوب لتجنب تضارب totalPrice من API
  ...
  // ✅ إظهار اسم الكلاس داخل label بدون تعديل HTML
  const className = sessionStorage.getItem("selectedClassName") || "—";
  const taxesRowLabel = $("taxes")?.parentElement?.querySelector("span");
  if (taxesRowLabel) taxesRowLabel.textContent = `Class Fees (${className})`;
...
async function ensureClientId() {
  ...
  if (!personID) throw new Error("PersonID missing. Login again and try.");

  const c = await apiGet(ENDPOINTS.clientByPerson(personID));

  const clientID =
    Number(c?.clientId ?? c?.clientID ?? c?.ClientID ?? c?.id ?? c?.ID ?? 0) ||
    0;
  ...
  const docId =
    Number(
      c?.documentationId ?? c?.documentationID ?? c?.DocumentationID ?? 0,
    ) || 0;
...
async function createBookingTrip() {
  ...
  const tripTypeID = Number(sessionStorage.getItem("tripTypeID") || "1") || 1;
  ...
  if (!bookId) {
    throw new Error("BookingTrip created but BookingTripID not returned.");
  }
...
async function getOrCreateTicket(bookId) {
  ...
  if (passengerClassID < 1) {
    throw new Error("PassengerClassID missing. Go back and select class.");
  }
  ...
  const body = {
    ...
    passengerClassID,
    passengersCount,
    ticketPrice: Number(sessionStorage.getItem("flightTotal") || "0") || 0,
    isActive: true,
  ...
  if (!ticketId) throw new Error("Ticket created but TicketId not returned.");
  return ticketId;
}

// ✅ أهم تعديل: FlightType لازم يكون Outbound / Return / Leg ...
...
async function load() {
  ...
  try {
    ...
    const schedules = await loadSchedulesForSegments(itinerary.segments || []);
    if (!schedules.length) {
      throw new Error(
...
$("seatBtn")?.addEventListener("click", () => {
  ...
  if (!first) return;
  window.location.href =
    "SeatMap.html?id=" + encodeURIComponent(String(first.flightScheduleId));
});
