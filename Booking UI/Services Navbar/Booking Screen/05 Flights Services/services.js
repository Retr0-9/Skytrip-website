...
// محاولات Endpoints للتكت/سيرفيسز (Fallback)
...
function getTSServiceId(obj) {
  return Number(
    obj?.serviceID ?? obj?.ServiceID ?? obj?.serviceId ?? obj?.ServiceId ?? 0,
  );
}

function getTSQty(obj) {
  return Number(obj?.quantity ?? obj?.Quantity ?? obj?.qty ?? obj?.Qty ?? 0);
}
...
function pickIconByName(name) {
  ...
  if (s.includes("meal") || s.includes("food")) return "🍽️";
  if (s.includes("wheel") || s.includes("chair")) return "♿";
  if (s.includes("assist")) return "🧑‍✈️";
 ...
 ************************************************************/
const ticketId =
  Number(qs("ticketId") || sessionStorage.getItem("ticketId") || "0") || 0;
const bookId =
...
async function loadTicketServices() {
  ...
  if (!r.ok) {
    console.warn("TicketServices GET failed:", r.error?.message || r.error);
    return;
  }
...
async function goToPayment() {
  ...
  const currency = (sessionStorage.getItem("currency") || "jod").toLowerCase();
